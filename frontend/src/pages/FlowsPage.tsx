/**
 * Flows Page — Coordinator Command Center
 *
 * Features:
...
export function FlowsPage() {
  ...
  const [remindingFlowId, setRemindingFlowId] = useState<string | null>(null);
  const [remindedFlowId, setRemindedFlowId] = useState<string | null>(null);
  const [actingFlowId, setActingFlowId] = useState<string | null>(null);
  ...
  // Lookup maps
  const attentionByFlowId = new Map(attentionItems.map((a) => [a.flow.id, a]));
  const filteredAttention = filterByAttentionSettings(attentionItems, attentionSettings);
  const filteredAttentionFlowIds = new Set(filteredAttention.map((a) => a.flow.id));
  ...
  const handleRemind = async (run: Flow) => {
    ...
    setRemindingFlowId(run.id);
    setRemindedFlowId(null);
    try {
      await remindStep(run.id, run.currentStep.stepId);
      setRemindedFlowId(run.id);
      setTimeout(() => setRemindedFlowId(null), 2000);
    } catch {
      setActionError('Failed to send reminder.');
    } finally {
      setRemindingFlowId(null);
    }
  };

  const handleAct = async (run: Flow) => {
    if (!run.currentStep?.stepId) return;
    setActingFlowId(run.id);
    try {
      ...
      setActingFlowId(null);
    }
  };

  // Bulk remind handler
  const handleBulkRemind = async () => {
    const overdueFlowIds = filteredRuns
      .filter((run) => {
        const attn = attentionByFlowId.get(run.id);
        return attn && attn.reasons.some((r) => r.type === 'STEP_OVERDUE' || r.type === 'FLOW_OVERDUE');
      })
      .map((run) => run.id);

    if (overdueFlowIds.length === 0) return;

    setIsBulkReminding(true);
    setBulkRemindResult(null);
    try {
      const result = await bulkRemind(overdueFlowIds);
  ...
  const filteredRuns = runs.filter((run) => {
    ...
    if (flowFilter === 'involved') {
      if (!filteredAttentionFlowIds.has(run.id)) return false;
    }

    // Template filter
    if (templateFilter !== 'all') {
      if (run.template?.id !== templateFilter && run.templateId !== templateFilter) return false;
    }
      ...
      {filteredRuns.length > 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
          {filteredRuns.map((run) => {
            const attentionData = attentionByFlowId.get(run.id);
            const hasAttention = filteredAttentionFlowIds.has(run.id);
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    {hasAssignee && run.status === 'IN_PROGRESS' && (
                      <button
                        ...
                        disabled={remindingFlowId === run.id}
                      >
                        {remindingFlowId === run.id ? (
                          <Loader2 className="w-3.5 h-3.5 animate-spin" />
                        ) : remindedFlowId === run.id ? (
