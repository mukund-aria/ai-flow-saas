    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative flex flex-col w-full h-full bg-white m-4 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Left: Categories */}
          <div className="w-64 border-r border-gray-200 overflow-y-auto bg-gray-50 shrink-0">
            <div className="p-3">
              <button
                ...
                {filteredTemplates.map(template => (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {Array.from(new Set(template.steps.map(s => s.type))).slice(0, 4).map(type => {
                        const StepIcon = STEP_TYPE_ICONS[type] || FileText;
                        return (
                          <span key={type} className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium rounded ${STEP_TYPE_COLORS[type] || 'bg-gray-100 text-gray-600'}`}>
                            <StepIcon className="w-2.5 h-2.5" />
                            {type.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()).replace('File Request', 'File').replace('Acknowledgement', 'Ack')}
                          </span>
                        );
                      })}
                    </div>
          ...
          {selectedTemplate && (
            <div className="w-96 border-l border-gray-200 overflow-y-auto bg-white shrink-0">
              <div className="p-6">
                {/* Header */}
                <div className="mb-6">
                ...
                {selectedTemplate.trigger && (
                  <div className="mb-5">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">When to start</p>
                ...
                {/* Roles */}
                <div className="mb-5">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Roles ({selectedTemplate.roles.length})</p>
                  <div className="flex flex-wrap gap-2">
                    {selectedTemplate.roles.map(role => (
                      <span key={role} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-violet-50 text-violet-700 text-xs font-medium rounded-lg">
                        <Users className="w-3 h-3" />
                        {role}
                      </span>
                    ))}
                      ...
                      return (
                        <div key={i} className="flex items-start gap-3 group">
                          <div className="flex flex-col items-center">
                            <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${STEP_TYPE_COLORS[step.type] || 'bg-gray-100 text-gray-600'}`}>
                ...
                {/* Import Button */}
                <Button
                  className="w-full bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 h-11 text-base gap-2"
                ...
                <p className="text-xs text-gray-400 text-center mt-2">
                  Creates a copy in your org as a draft template
                </p>
              </div>
            </div>
          )}
