...
function DraggablePaletteItem({ type }: { type: StepType }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">{meta.label}</span>
      {isComingSoon ? (
...
function MilestonePaletteItem() {
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full flex items-center gap-2.5 px-3 py-2 rounded-md hover:bg-gray-50 transition-colors text-left group"
    >
      <div className="w-7 h-7 rounded flex items-center justify-center shrink-0 bg-gray-100">
        <Flag className="w-4 h-4 text-gray-600" />
      </div>
      <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">Milestone</span>
...
export function StepPalette({ onSwitchToAI }: StepPaletteProps) {
  ...
  return (
    <div className="w-[240px] shrink-0 bg-white border-r border-gray-200 flex flex-col h-full overflow-hidden">
      {/* Header */}
