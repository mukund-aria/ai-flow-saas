...
**After implementing any plan or multi-file change, ALWAYS run the global `first-principles-review` skill before declaring work complete.** This catches UX issues, code quality problems, accessibility gaps, and consistency violations proactively — the user should never have to find these issues themselves.

Project-specific principles (in addition to the global skill checklist):
...
- **Color consistency:** Use `violet` for primary accent — never `purple`. Use design tokens (`meta.color`, `getRoleColor()`).
- **No duplicate information:** Don't show the same data (title, status) in multiple places on screen.
- **No placeholder UI:** Don't ship components with `href="#"` or non-functional buttons. Either wire them up or don't render them.
